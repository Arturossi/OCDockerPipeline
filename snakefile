...
pipeline_save_json_descriptors = _as_bool(config.get("pipeline_save_json_descriptors", False), default=False)
...
def _receptor_descriptor_json_path(database: str, receptor: str) -> Path:
    '''
    Return receptor descriptor JSON sidecar path for one receptor entry.
    ...
    -------
    Path
        Path to ``receptor_descriptors.json``.
    '''

    return _database_rule_root_path(database) / receptor / "receptor_descriptors.json"
...
def _ligand_descriptor_json_path(database: str, receptor: str, kind: str, target: str) -> Path:
    '''
    Return ligand descriptor JSON sidecar path for one target entry.
    ...
    -------
    Path
        Path to ``ligand_descriptors.json``.
    '''

    return _target_dir_path(database, receptor, kind, target) / "ligand_descriptors.json"
...
pipeline_batch_enabled = config.get("pipeline_batch_index") not in (None, "")
if pipeline_batch_enabled:
    database_to_receptors: Dict[str, List[str]] = {database: [] for database in selected_databases}
else:
...
def _load_target_discovery_cache_unchecked() -> Optional[Tuple[List[str], int]]:
    '''
    Load cached discovered targets without recomputing the filesystem signature.
    '''
...
def _load_target_discovery_cache_batch_unchecked(
    batch_index: int,
    total_batches: int,
) -> Optional[Tuple[List[str], int, int]]:
    '''
    Load only one batch slice from cached discovered targets.
    '''

    cached = _load_target_discovery_cache_unchecked()
    if cached is None:
        return None

    targets, scanned = cached
    total_targets = len(targets)
    if batch_index > total_batches:
        raise ValueError(
            f"pipeline_batch_index={batch_index} exceeds pipeline_total_batches={total_batches}."
        )
    start = (batch_index - 1) * total_targets // total_batches
    end = batch_index * total_targets // total_batches
    sliced = targets[start:end]
    if not sliced:
        raise ValueError(
            f"Pipeline batch {batch_index}/{total_batches} selected no targets from {total_targets} total targets."
        )
    return sliced, scanned, total_targets
...
_payload_targets_already_sliced = False
...
def collect_payload_targets():
    ...
    global _payload_targets_already_sliced
    ...
    if pipeline_discovery_cache:
        batch_index = _as_optional_positive_int(config.get("pipeline_batch_index"), "pipeline_batch_index")
        total_batches = _as_optional_positive_int(config.get("pipeline_total_batches"), "pipeline_total_batches")
        if batch_index is not None and total_batches is not None:
            cached = _load_target_discovery_cache_batch_unchecked(batch_index, total_batches)
            if cached is not None:
                cached_targets, cached_scanned, cached_total = cached
                _payload_targets_already_sliced = True
                print(
                    "Discovery summary: "
                    f"mode={target_discovery_mode}, scanned={cached_scanned}, valid_targets={cached_total}, "
                    "cache=trusted-batch-hit"
                )
                print(
                    "Pipeline batch summary: "
                    f"batch={batch_index}/{total_batches}, targets={len(cached_targets)}, total_targets={cached_total}"
                )
                return cached_targets
            raise RuntimeError(
                "Pipeline batch mode could not load a usable target discovery cache. "
                f"Expected cache at {_target_discovery_cache_path()}. "
                "Refusing to fall back to full target discovery/listing because that would "
                "make this batch build the global DAG."
            )
...
def _as_optional_positive_int(value: Any, name: str) -> Optional[int]:
    '''
    ...
    try:
        ...
        raise ValueError(f"{name} must be a positive integer when provided.") from exc
    if parsed <= 0:
        raise ValueError(f"{name} must be a positive integer when provided.")
    return parsed


def _slice_payload_targets_for_config(targets: List[str]) -> List[str]:
    '''
    Apply pipeline-level batching before rule all is constructed.
    '''

    # collect_payload_targets() may already return a sliced list for cached batched runs.
    if (
        pipeline_discovery_cache
        and config.get("pipeline_batch_index") not in (None, "")
        and _payload_targets_already_sliced
    ):
        return targets

    batch_index = _as_optional_positive_int(config.get("pipeline_batch_index"), "pipeline_batch_index")
    total_batches = _as_optional_positive_int(config.get("pipeline_total_batches"), "pipeline_total_batches")
    batch_size = _as_optional_positive_int(config.get("pipeline_batch_size"), "pipeline_batch_size")

    if batch_index is None and total_batches is None and batch_size is None:
        return targets

    if batch_index is None:
        raise ValueError("pipeline_batch_index is required when pipeline batching is enabled.")
    if total_batches is not None and batch_size is not None:
        raise ValueError("Use only one of pipeline_total_batches or pipeline_batch_size.")
    if total_batches is None and batch_size is None:
        raise ValueError("pipeline_total_batches or pipeline_batch_size is required with pipeline_batch_index.")
    ...
    print(
        "Pipeline batch summary: "
        f"batch={batch_index}/{total_batches}, targets={len(sliced)}, total_targets={total_targets}"
    )
    return sliced


all_payload_targets = _slice_payload_targets_for_config(collect_payload_targets())
...
def _write_receptor_descriptor_json(receptor_path: Union[str, Path], descriptor_json: Union[str, Path]) -> None:
    '''
    Compute and persist one receptor descriptor JSON sidecar.

    Parameters
    ----------
    receptor_path : Union[str, Path]
        Receptor PDB path.
    descriptor_json : Union[str, Path]
        Expected ``receptor_descriptors.json`` output path.

    Returns
    -------
    None
        This function writes the descriptor JSON output.
    '''
    ...
    with _file_lock(descriptor_json.with_name(f".{descriptor_json.name}.lock")):
        ...
        if not _is_valid_file(descriptor_json):
            raise RuntimeError(f"Failed to create receptor descriptor JSON at '{descriptor_json}'.")


def _write_ligand_descriptor_json(ligand_path: Union[str, Path], descriptor_json: Union[str, Path]) -> None:
    '''
    Compute and persist one ligand descriptor JSON sidecar.

    Parameters
    ----------
    ligand_path : Union[str, Path]
        Primary ligand input path.
    descriptor_json : Union[str, Path]
        Expected ``ligand_descriptors.json`` output path.
    ...
    descriptor_json = Path(descriptor_json)
    target_dir = descriptor_json.parent
    target_dir.mkdir(parents=True, exist_ok=True)
    ...
    with _file_lock(descriptor_json.with_name(f".{descriptor_json.name}.lock")):
        for candidate in candidates:
            try:
                ligand_obj = ocl.Ligand(str(candidate), name="ligand")
                ligand_obj.to_json(overwrite=True)
                if _is_valid_file(descriptor_json):
                    return
                errors.append(f"{candidate}: descriptor JSON was not written")
            except Exception as exc:
                errors.append(f"{candidate}: {type(exc).__name__}: {exc}")

    details = "; ".join(errors) if errors else "no ligand candidates found"
    raise RuntimeError(f"Failed to create ligand descriptor JSON at '{descriptor_json}': {details}")
...
def _ligand_group_id(wildcards) -> str:
    '''
    Group id for the per-target job chain (box/cache/descriptor/engines/
    postprocess/oddt/finalize) of a single ligand entry.

    Each rule `run:` block is dispatched by Snakemake as its own subprocess
    (full Python+Snakemake startup per job); with hundreds of thousands of
    ligand entries, that per-job overhead dominates wall time far more than
    the actual docking/rescoring work. Grouping every stage of one ligand's
    chain under a single group id lets Snakemake execute them in one
    subprocess instead of ~9, without changing rule semantics or output
    paths -- each rule's own input/output declarations are unchanged, so
    already-completed outputs are still detected and skipped normally.
    ...
    -------
    str
        Group id unique to this (database, receptor, kind, target) entry.
    '''

    return f"ligand_{wildcards.database}_{wildcards.receptor}_{wildcards.kind}_{wildcards.target}"


def _wc_receptor_descriptor_json_path(wildcards) -> Union[str, List[str]]:
    '''
    Resolve receptor descriptor JSON output path from Snakemake wildcards.
    ...
    -------
    str
        Receptor descriptor JSON path for current wildcard tuple.
    '''

    if not pipeline_save_json_descriptors:
        return []
    return str(_receptor_descriptor_json_path(wildcards.database, wildcards.receptor))
...
def _wc_ligand_descriptor_json_path(wildcards) -> Union[str, List[str]]:
    '''
    Resolve ligand descriptor JSON output path from Snakemake wildcards.
    ...
    -------
    str
        Ligand descriptor JSON path for current wildcard tuple.
    '''

    if not pipeline_save_json_descriptors:
        return []
    return str(_ligand_descriptor_json_path(wildcards.database, wildcards.receptor, wildcards.kind, wildcards.target))
...
rule receptor_descriptor_json:
    """
    Compute receptor descriptor JSON sidecar for reuse by CSV/DB workflows.
    """
    input:
        receptor=_wc_receptor_path,
    output:
    ...
    run:
        try:
            _write_receptor_descriptor_json(
                receptor_path=str(input.receptor),
                descriptor_json=str(output.json),
            )
        except Exception as exc:
            _append_rule_failure_log(log[0], "receptor_descriptor_json", wildcards, exc)
            raise


rule ligand_descriptor_json:
    """
    Compute ligand descriptor JSON sidecar for reuse by CSV/DB workflows.
    """
    input:
        ligand=_wc_ligand_path,
    output:
        json=os.path.join(
            ...
            "{kind}",
            "{target}",
            "ligand_descriptors.json",
        ),
    log:
        str(
            PIPELINE_RULE_FAILURE_LOG_DIR
            / "ligand_descriptor_json"
            / "{database}"
            / "{receptor}"
            / "{kind}"
            / "{target}.log"
    ...
    run:
        try:
            _write_ligand_descriptor_json(
                ligand_path=str(input.ligand),
                descriptor_json=str(output.json),
            )
        except Exception as exc:
            _append_rule_failure_log(log[0], "ligand_descriptor_json", wildcards, exc)
            raise
...
rule run_pipeline:
    ...
    input:
        receptor=_wc_receptor_path,
        receptor_cache=_wc_receptor_cache_manifest_path,
        receptor_descriptor_json=_wc_receptor_descriptor_json_path,
        ligand=_wc_ligand_path,
        ligand_descriptor_json=_wc_ligand_descriptor_json_path,
        box=_wc_box_path,
        engine_summaries=_engine_summary_inputs,
        core_summary=_wc_core_summary_path,
